...
/*
Query to group service types together along with the count of each value
...
SELECT
    ...
    CASE
        ...
        WHEN LENGTH(handle.id)= 2 AND handle.id LIKE '+1%' THEN '('||SUBSTR(handle.id,3,3)||')'||SUBSTR(handle.id,6,3)||'-'||SUBSTR(handle.id,9,4)
...
WHERE
    ...
    /* When `chat_message_join.chat_id` is NULL, that means that the message
    was deleted, but there is still a record in the message table */
